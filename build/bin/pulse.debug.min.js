...
this.tabs={};this.tabbarLinks={};this.fpsTimer=a.fpsTimer;this.drawCounter=a.drawCounter;this.nodeCounter=a.nodeCounter;a.useDefault&&(this.addTab(new pulse.debug.tabs.Console({name:"Console",id:"console"})),a=new pulse.debug.tabs.Performance({name:"Performance",id:"perf",fpsTimer:a.fpsTimer,updateTimer:a.updateTimer,drawTimer:a.drawTimer}),this.addTab(a),this.addTab(new pulse.debug.tabs.Inspector({name:"Inspector",id:"inspector"})),this.showTab(a.id))},addTab:function(a){this.tabs[a.id]=a;var b=
this,c=document.createElement("a");c.href="#";c.innerHTML=a.name;c.onclick=function(){b.showTab(a.id);return false};if(a.icon!=="")c.style.backgroundImage="url("+a.icon+")";c.id="pulse-tab-link-"+a.id;c.name=a.name;c.style.cssText="height: 30px; padding: 0px 10px; line-height: 30px; display: inline-block; text-decoration: none; color: #ccc;";a.container.style.display="none";this.tabbarLinks[a.id]=c;this.tabbar.appendChild(c);this.tabholder.appendChild(a.container)},removeTab:function(a){this.tabs[a]&&
(this.tabholder.removeChild(this.tabs[a]),this.tabbar.removeChild(this.tabbarLinks[a]),delete this.tabs[a],delete this.tabbarLinks[a])},showTab:function(a){for(var b in this.tabbarLinks)if(this.tabbarLinks[b].className!=="")this.tabbarLinks[b].className="",this.tabbarLinks[b].style.backgroundColor="",this.tabbarLinks[b].style.color="#ccc",this.tabs[b].container.className="debug-tab",this.tabs[b].hide();if(this.tabs[a])b=this.tabs[a],a=this.tabbarLinks[a],a.className="selected",a.style.backgroundColor=
"#333",a.style.color="#CCFF00",b.container.className="debug-tab selected",b.show()},resize:function(a){this.tabholder.style.height=a+"px";for(var b in this.tabs)this.tabs[b].resize(a)},logMessage:function(a,b){this.tabs.console&&this.tabs.console.logMessage(a,b)},setEngine:function(a){this.tabs.inspector&&this.tabs.inspector.setEngine(a)},addNode:function(a){this.tabs.inspector&&this.tabs.inspector.addNode(a)},removeNode:function(a){this.tabs.inspector&&this.tabs.inspector.removeNode(a)},update:function(a){for(var b in this.tabs){if(this.tabs[b].visible===
true)this.tabs[b].container.style.display="block";else if(this.tabs[b].container.style.display!=="none")this.tabs[b].container.style.display="none";this.tabs[b].update(a)}if(this.fpsTimer)this.statusMS.innerHTML=Math.round(this.fpsTimer.markCurrent*100)/100+"ms",this.statusFPS.innerHTML="FPS: "+this.fpsTimer.fps;if(this.drawCounter)this.statusDraws.innerHTML="Draws: "+this.drawCounter.count;if(this.nodeCounter)this.statusNodes.innerHTML="Nodes: "+this.nodeCounter.count}});
...
"-value",d.style.cssText="color: #CCC;",a.appendChild(d),this._private.nodePropsDiv.appendChild(a);this._private.engine=null},show:function(){this._super()},hide:function(){this._super()},update:function(){},setEngine:function(a){this._private.engine=a;for(var a=a.scenes.getScenes(true),b=0;b<a.length;b++)this.styleVisibility(a[b])},getNodeDiv:function(a,b,c){var d=document.createElement("div");d.id="inspector-node-"+a.name;d.style.cssText="padding: 0 2px;";var e=document.createElement("span");e.className=
"name";e.style.cssText="cursor: pointer; padding: 2px;";e.innerHTML=a.name;var f=this;e.onclick=function(){f.selectNode(a)};var g=null;if(b===true){g=document.createElement("div");g.id="inspector-"+a.name+"-children";g.style.marginLeft=c+"px";g.style.display="none";var h=document.createElement("div");h.style.cssText="display: inline-block; width: 20px; height: 20px; font-size: 14px; cursor: pointer;";h.onclick=function(){f.toggleVisibility(g);h.innerHTML=g.style.display==="block"?"&#9660;":"&#9658;"};
h.innerHTML="&#9658;";d.appendChild(h);d.appendChild(e);d.appendChild(g);e.onclick=function(){f.selectNode(a)}}else d.appendChild(e);return{div:d,container:g}},addNode:function(a){a.debugging=false;var b=null;a.parent!==null&&(b=document.getElementById("inspector-"+a.parent.name+"-children"));var c=null;if(a instanceof pulse.Scene){c=this.getNodeDiv(a,true,10);this._private.nodeList.appendChild(c.div);for(var d in a.layers)this.addNode(a.layers[d])}else if(a instanceof pulse.Layer&&b!==null){c=this.getNodeDiv(a,
true,30);b.appendChild(c.div);for(var e in a.objects)this.addNode(a.objects[e])}else b!==null&&(c=this.getNodeDiv(a,false),b.appendChild(c.div));this.styleVisibility(a)},removeNode:function(a){var b=a;if(a instanceof pulse.Node)b=a.name;a=document.getElementById("inspector-node-"+b);a!==null&&a.parentNode.removeChild(a)},selectNode:function(a){var b;if(this.selectedNode!==null&&(b=document.getElementById("inspector-node-"+this.selectedNode.name),b!==null)){var c=b.children[0];b.children[0].className!==
"name"&&(c=b.children[1]);c.style.color=b.style.color;c.style.background="transparent"}this.selectedNode=a;b=document.getElementById("inspector-node-"+a.name);if(b!==null)c=b.children[0],b.children[0].className!=="name"&&(c=b.children[1]),c.style.background=b.style.color,c.style.color="#222";for(var d in this._private.nodeProps)if(b=this._private.nodeProps[d],c=document.getElementById("inspector-prop-"+b.toLowerCase()+"-value"),c!==null)c.innerHTML=a.hasOwnProperty(b.toLowerCase())?this.getValueString(a[b.toLowerCase()]):
"N/A";this.visibilityAction.innerHTML=a.visible?"HIDE":"SHOW";this.outlineAction.style.cssText+=a.debugging?"background: #444;":"background: #666;"},toggleNode:function(a){a instanceof pulse.Scene&&this._private.engine instanceof pulse.Engine&&(a.active===true?this._private.engine.scenes.deactivateScene(a.name):this._private.engine.scenes.activateScene(a.name));if(a instanceof pulse.Visual)a.visible=a.visible===true?false:true;this.styleVisibility(a);this.selectNode(a)},styleVisibility:function(a){var b=
document.getElementById("inspector-node-"+a.name);if(b!==null){var c=pulse.debug.getTypeColor(a);if(a.visible===false||a.active===false)c=this.disableColor(c);b.highlightColor="#"+c;b.style.color="#"+c}},toggleDebug:function(a){a.debugging=a.debugging===true?false:true;this.selectNode(a)},toggleVisibility:function(a){a.style.display=a.style.display!=="none"?"none":"block"},getValueString:function(a){var b="";if(typeof a==="object"){for(var c in a)b+=this.getValueString(a[c])+", ";b=b.substring(0,
b.length-2)}else b+=typeof a==="number"?a.toFixed(2):a;return b},disableColor:function(a){var b=parseInt(a,16),a=(b&16711680)>>16,c=(b&65280)>>8,b=(b&255)>>0;a+=Math.floor(-100/255*a);c+=Math.floor(-100/255*c);b+=Math.floor(-100/255*b);return((a<<16)+(c<<8)+b).toString(16)},resize:function(a){this._private.nodeList.style.height=a+"px";this._private.nodePropsDiv.style.height=a+"px"}});pulse.debug.DebugManager=PClass.extend({init:function(){this.timers={};this.counters={};this.fps=0;this.running=false;this._private={};this._private.fpsTimer=new pulse.debug.FPS;this._private.fpsTimer.start();this._private.updateTimer=new pulse.debug.Timer;this._private.drawTimer=new pulse.debug.Timer;this._private.drawCounter=new pulse.debug.Counter;this._private.nodeCounter=new pulse.debug.Counter;this.panel=null;this.setupPanel()},logWarning:function(a){this.panel.logMessage(a,"warning")},logDebug:function(a){this.panel.logMessage(a,
"debug")},logError:function(a){this.panel.logMessage(a,"error")},markTime:function(a){this.timers[a]?this.timers[a].mark():(this.timers[a]=new pulse.debug.DebugTimer({name:a}),this.timers[a].start())},markFPS:function(){this._private.fpsTimer.mark();this.fps=this._private.fpsTimer.fps},startUpdate:function(){this._private.updateTimer.start()},stopUpdate:function(){this._private.updateTimer.mark()},startDraw:function(){this._private.drawTimer.start()},stopDraw:function(){this._private.drawTimer.mark()},
...
drawTimer:this._private.drawTimer,drawCounter:this._private.drawCounter,nodeCounter:this._private.nodeCounter})},update:function(a){this.panel.update(a)}});pulse.debug.visualDebug=false;pulse.debug.manager=new pulse.debug.DebugManager;pulse.debug.plugin=new pulse.plugin.Plugin;var loopCallback=pulse.debug.plugin.subscribe("pulse.Engine","loop",pulse.plugin.PluginCallbackTypes.onEnter,function(){pulse.debug.manager.setEngine(this);pulse.debug.plugin.unsubscribe(loopCallback)});pulse.debug.plugin.subscribe("pulse.Engine","update",pulse.plugin.PluginCallbackTypes.onEnter,function(){pulse.debug.manager.markFPS();pulse.debug.manager.startUpdate()});
pulse.debug.plugin.subscribe("pulse.Engine","update",pulse.plugin.PluginCallbackTypes.onExit,function(){pulse.debug.manager.stopUpdate()});pulse.debug.plugin.subscribe("pulse.Engine","draw",pulse.plugin.PluginCallbackTypes.onEnter,function(){pulse.debug.manager.resetDraws();pulse.debug.manager.startDraw()});pulse.debug.plugin.subscribe("pulse.Engine","draw",pulse.plugin.PluginCallbackTypes.onExit,function(){pulse.debug.manager.stopDraw();pulse.debug.manager.update(this.masterTime)});
pulse.debug.plugin.subscribe("pulse.Visual","draw",pulse.plugin.PluginCallbackTypes.onEnter,function(){this.updated===true&&pulse.debug.manager.incrementDraws()});
pulse.debug.plugin.subscribe("pulse.Visual","draw",pulse.plugin.PluginCallbackTypes.onExit,function(a){if(this.debugging===true||pulse.debug.visualDebug===true){var b="#"+pulse.debug.getTypeColor(this);a.save();a.fillStyle=b;a.beginPath();a.arc(this.positionTopLeft.x+this.canvas.width/2,this.positionTopLeft.y+this.canvas.height/2,3,0,Math.PI*2,true);a.closePath();a.fill();a.restore();a.strokeStyle=b;a.strokeRect(this.positionTopLeft.x/Math.abs(this.scale.x),this.positionTopLeft.y/Math.abs(this.scale.y),
this.size.width,this.size.height)}});pulse.debug.plugin.subscribe("pulse.Layer","addNode",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.addNode(a)});pulse.debug.plugin.subscribe("pulse.Layer","removeNode",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.removeNode(a)});pulse.debug.plugin.subscribe("pulse.Scene","addLayer",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.addNode(a)});
pulse.debug.plugin.subscribe("pulse.Scene","removeLayer",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.removeNode(a)});pulse.debug.plugin.subscribe("pulse.SceneManager","addScene",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.addNode(a)});pulse.debug.plugin.subscribe("pulse.SceneManager","removeScene",pulse.plugin.PluginCallbackTypes.onExit,function(a){pulse.debug.manager.removeNode(a)});pulse.plugins.add(pulse.debug.plugin);
pulse.log=function(a,b){b==="error"?pulse.debug.manager.logError(a):b==="warning"?pulse.debug.manager.logWarning(a):pulse.debug.manager.logDebug(a)};pulse.info=function(a){pulse.debug.manager.logDebug(a)};pulse.warn=function(a){pulse.debug.manager.logWarning(a)};pulse.error=function(a){pulse.debug.manager.logError(a)};
