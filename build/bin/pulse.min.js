...
pulse.plugin.PluginCallback=PClass.extend({init:function(a){this.objectType=a.objectType;this.functionName=a.functionName;this.callbackType=a.callbackType;this.callback=a.callback}});pulse.plugin.PluginCallbackTypes={onEnter:"onEnter",onExit:"onExit"};pulse=pulse||{};pulse.plugin=pulse.plugin||{};pulse.plugin.PluginCollection=PClass.extend({init:function(){this._private={};this._private.plugins=[]},add:function(a){this._private.plugins.push(a)},remove:function(a){for(var b in this._private.plugins)this._private.plugins[b]===a&&this._private.plugins.splice(b,1)},invoke:function(){for(var a in this._private.plugins)this._private.plugins[a].invoke.apply(this._private.plugins[a],arguments)}});pulse.plugins=pulse.plugins||new pulse.plugin.PluginCollection;pulse=pulse||{};pulse.error={DuplicateName:function(a){throw"There is already an object with the name "+a+" on this layer.";},InvalidSource:function(){throw"Invalid source for pulse image.";}};pulse=pulse||{};pulse.util=pulse.util||{};pulse.util.find=function(a,b){var e=[];if(a instanceof Array)for(var f=0;f<a.length;f++)typeof a[f]=="function"&&typeof a[f].name=="function"&&a[f].name==b&&e.push(a[f]);else if(typeof a=="object")for(f in a)f==b&&e.push(a[f]);return e};pulse.util.intersects=function(a,b){for(var e=[{x:a.x,y:a.y},{x:a.x,y:a.y+a.height},{x:a.x+a.width,y:a.y},{x:a.x+a.width,y:a.y+a.height}],f=0;f<4;f++){var g=e[f];if(g.x<b.width&&g.x>b.x&&g.y<b.height&&g.y>b.y)return true}return false};
...
this.updated=true;if(this.invalidProperties)this.calculateProperties(),this.updated=true;pulse.plugins.invoke("pulse.Visual","update",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},draw:function(a){pulse.plugins.invoke("pulse.Visual","draw",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);if(!(this.canvas.width===0||this.canvas.height===0)){a.save();a.globalAlpha=this.alpha/100;if(this.rotation!==0){var b=this.positionTopLeft.x+this.size.width*Math.abs(this.scale.x)/2,e=this.positionTopLeft.y+
this.size.height*Math.abs(this.scale.y)/2;a.translate(b,e);a.rotate(Math.PI*(this.rotation%360)/180);a.translate(-b,-e)}a.scale(this.scale.x,this.scale.y);b=this.positionTopLeft.x/this.scale.x;this.scale.x<1&&(b-=this.size.width);e=this.positionTopLeft.y/this.scale.y;this.scale.y<1&&(e-=this.size.height);if(this.shadowEnabled)a.shadowOffsetX=this.shadowOffsetX,a.shadowOffsetY=this.shadowOffsetY,a.shadowBlur=this.shadowBlur,a.shadowColor=this.shadowColor;a.drawImage(this.canvas,b,e);a.restore();this.updated=
false;pulse.plugins.invoke("pulse.Visual","draw",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)}},calculateProperties:function(){var a=this.size.width,b=this.size.height,a=this.anchor.x*a-a/2,b=this.anchor.y*b-b/2;this.anchorRadius=Math.sqrt(a*a+b*b);this.anchorAngle=Math.atan2(b,a)*180/Math.PI-90;if(isNaN(this.anchorAngle))this.anchorAngle=0;this.positionTopLeftPrevious=this.positionTopLeft;a=this.size.width*Math.abs(this.scale.x)/2;b=this.size.height*Math.abs(this.scale.y)/2;a=this.position.x-
Math.sin(Math.PI*-(this.rotation+this.anchorAngle)/180)*this.anchorRadius-a;b=this.position.y-Math.cos(Math.PI*-(this.rotation+this.anchorAngle)/180)*this.anchorRadius-b;this.positionTopLeft={x:a,y:b};if(this.canvas.width!==this.size.width)this.canvas.width=this.size.width;if(this.canvas.height!==this.size.height)this.canvas.height=this.size.height;this.boundsPrevious=this.bounds;this.bounds={x:this.positionTopLeft.x,y:this.positionTopLeft.y,width:this.size.width*Math.abs(this.scale.x),height:this.size.height*
Math.abs(this.scale.y)};this.invalidProperties=false},on:function(a,b){this.events.bind(a,b)},eventsCallback:function(){}});pulse=pulse||{};
...
this,arguments)},removeNode:function(a){pulse.plugins.invoke("pulse.Layer","removeNode",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);var b=a;if(a instanceof pulse.Visual)b=a.name;if(this.objects.hasOwnProperty(b)){if(this.objects[b]instanceof pulse.Visual){var e=this.objects[b].boundsPrevious;this._private.context.clearRect(e.x,e.y,e.width,e.height)}this.objects[b].parent=null;delete this.objects[b]}pulse.plugins.invoke("pulse.Layer","removeNode",pulse.plugin.PluginCallbackTypes.onExit,
this,arguments)},getNode:function(a){return this.objects[a]},getNodesByType:function(a){var b={},e;for(e in this.objects)this.objects[e]instanceof a&&(b[e]=this.objects[e]);return b},update:function(a){pulse.plugins.invoke("pulse.Layer","update",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);var b=false,e;for(e in this.objects)if(this.objects[e]instanceof pulse.Visual){if(this.objects[e].shuffled===true)this.objects[e].shuffled=false,b=true;this.objects[e].update(a);if(this.objects[e].updated)this.updated=
true}if(b)this._private.orderedKeys=pulse.util.getOrderedKeys(this.objects);this._super(a);pulse.plugins.invoke("pulse.Layer","update",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},draw:function(a){pulse.plugins.invoke("pulse.Layer","draw",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);var b=false,e;for(e in this.objects)if(this.objects[e].updated){b=true;break}if(b){this._private.context.clearRect(0,0,this.size.width,this.size.height);for(b=0;b<this._private.orderedKeys.length;b++)if(e=
this.objects[this._private.orderedKeys[b]],e instanceof pulse.Visual)e.visible?e.draw(this._private.context):e.updated=false}this._super(a);pulse.plugins.invoke("pulse.Layer","draw",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},pointInBounds:function(a){return a.x>this.bounds.x&&a.x<this.bounds.x+this.bounds.width&&a.y>this.bounds.y&&a.y<this.bounds.y+this.bounds.height},eventsCallback:function(a,b){if(!(typeof b==="undefined"||typeof b.position==="undefined")){b.parent.x=b.position.x;
b.parent.y=b.position.y;var e=this.getNodesByType(pulse.Sprite),f,g;for(g in e)if(f=e[g],f.visible)if(pulse.events[a]==="mouse"||pulse.events[a]==="touch"){var i=f.bounds;b.position.x=b.parent.x-i.x;b.position.y=b.parent.y-i.y;b.sender=f;if(f.handleAllEvents||f.inCurrentBounds(b.parent.x,b.parent.y)){if(f.inCurrentBounds(b.parent.x,b.parent.y)&&f.mousein===false)f.mousein=true,f.events.raiseEvent("mouseover",b);f.events.raiseEvent(a,b)}if(!f.inCurrentBounds(b.parent.x,b.parent.y)&&f.mousein===true)f.mousein=
false,f.events.raiseEvent("mouseout",b);if(f.dropAcceptEnabled)for(var h in pulse.EventManager.DraggedItems)if(i=pulse.EventManager.DraggedItems[h],f.inCurrentBounds(b.parent.x,b.parent.y))b.target=i,f.draggedOverItems[h]?(b.sender=f,f.events.raiseEvent("dragover",b)):(f.draggedOverItems[h]=i,i.events.bind("dragdrop",f.itemDroppedCallback),f.events.raiseEvent("dragenter",b));else if(f.draggedOverItems[h])delete f.draggedOverItems[h],b.target=i,i.events.unbindFunction("dragdrop",f.itemDroppedCallback),
f.events.raiseEvent("dragexit",b)}else b.sender=f,f.events.raiseEvent(a,b)}}});pulse=pulse||{};
...
update:function(a){pulse.plugins.invoke("pulse.Scene","update",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);this._super(a);var b=false,e;for(e in this.layers)if(this.layers[e].shuffled===true)this.layers[e].shuffled=false,b=true;if(b===true)this._private.orderedKeys=pulse.util.getOrderedKeys(this.layers);for(var f in this.layers)this.layers[f].update(a);pulse.plugins.invoke("pulse.Scene","update",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},draw:function(){pulse.plugins.invoke("pulse.Scene",
"draw",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);for(var a=0;a<this._private.orderedKeys.length;a++){var b=this.layers[this._private.orderedKeys[a]];if(b.updated){var e=this._private.liveLayers[b.name].canvas,f=this._private.liveLayers[b.name].context;f.clearRect(0,0,e.width,e.height);b.visible===true?b.draw(f):b.updated=false}}pulse.plugins.invoke("pulse.Scene","draw",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},on:function(a,b){this.events.bind(a,b)},eventsCallback:function(a,
...
false,this.layers[e].events.raiseEvent("mouseout",b)}else b.sender=this.layers[e],this.layers[e].events.raiseEvent(a,b)}});pulse=pulse||{};
...
"loop",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);var b=this;(a||a===void 0)&&requestAnimFrame(function(){b.loop.call(b)},this._private.mainDiv);this._private.currentTime=(new Date).getTime();var e=this._private.currentTime-this._private.lastTime;if(!(e<this.tick)){var f=Math.floor(e/30);f===0&&(f=1);if(f<20){e/=f;for(var g=0;g<f;g++)this.loopLogic&&this.loopLogic(this.scenes,e),this.update(e),this.draw(),this.masterTime+=e}this._private.lastTime=this._private.currentTime;pulse.plugins.invoke("pulse.Engine",
"loop",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)}},update:function(a){pulse.plugins.invoke("pulse.Engine","update",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);for(var b=this.scenes.getScenes(true),e=0;e<b.length;e++)b[e].update(a);pulse.plugins.invoke("pulse.Engine","update",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},draw:function(){pulse.plugins.invoke("pulse.Engine","draw",pulse.plugin.PluginCallbackTypes.onEnter,this,arguments);for(var a=this.scenes.getScenes(true),
b=0;b<a.length;b++)a[b].draw();pulse.plugins.invoke("pulse.Engine","draw",pulse.plugin.PluginCallbackTypes.onExit,this,arguments)},windowEvent:function(a){if(!this.hidden){if(!a)a=window.event;var b=a.type,e=this.scenes.getScenes(true),f=this.getWindowOffset(),g=0,i=0;window.pageXOffset&&window.pageYOffset?(g=window.pageXOffset,i=window.pageYOffset):(g=document.body.scrollLeft,i=document.body.scrollTop);var h=new pulse.MouseEvent,d=a.clientX-f.x+g,l=a.clientY-f.y+i,p=false;if(pulse.events[b]==="touch"||
pulse.events[b]==="touchgesture"){p=true;h=new pulse.TouchEvent;if(a.touches&&a.touches.length>0)d=a.touches[0].clientX-f.x+g,l=a.touches[0].clientY-f.y+i,h.touches=a.touches;if(a.changedTouches&&a.changedTouches.length>0)h.changedTouches=a.changedTouches,b==="touchend"&&(d=a.changedTouches[0].clientX-f.x+g,l=a.changedTouches[0].clientY-f.y+i);if(a.targetTouches&&a.targetTouches.length>0)h.targetTouches=a.changedTouches;if(a.scale&&a.rotation)h.gestureScale=a.scale,h.gestureRotation=a.rotation;this.focused=
true}h.window.x=a.clientX;h.window.y=a.clientY;h.world.x=d;h.world.y=l;f=false;d>0&&d<parseInt(this._private.mainDiv.style.width,10)&&l>0&&l<parseInt(this._private.mainDiv.style.height,10)&&(f=true);for(g=0;g<e.length;g++){h.parent.x=h.window.x;h.parent.y=h.window.y;h.position.x=d;h.position.y=l;if(f){if(a.preventDefault&&p===false&&a.preventDefault(),document.activeElement!==this._private.mainDiv&&a.type.toLowerCase()==="click"&&(document.activeElement.blur(),this._private.mainDiv.focus()),a.type.toLowerCase()===
