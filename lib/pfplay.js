...
if (typeof PFPlay == 'undefined') {
  ...
  PFPlay = {
    ...
    init: function(libSrc) {
      PFPlay.libsrc = libSrc;
      var head = document.getElementsByTagName('head')[0];
 
      var files = [ ];
      for(var f in PFPlay.coreFiles) {
        files.push('order!' + libSrc + "/" + PFPlay.coreFiles[f]);
      }
      for(var f in PFPlay.gameFiles) {
        files.push('order!' + PFPlay.gameFiles[f]);
      }
      
      require(files, function() {
        PFPlay.isReady = true;
        setTimeout(PFPlay.notifyReady, 2000);
      });
    }
  };
}
...
PFPlay.libsrc = "";
PFPlay.isReady = false;
PFPlay.readyCallbacks = [ ];

PFPlay.ready = function(callback) {
  ...
  PFPlay.readyCallbacks.push(callback);
}

PFPlay.notifyReady = function() {
  // Notify anyone listening that the engine is now ready.
  for(var idx in PFPlay.readyCallbacks) {
    PFPlay.readyCallbacks[idx]();
  }  
}
