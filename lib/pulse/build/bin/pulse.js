...
pulse.support.codecs = {mp3:false, ogg:false, wav:false, m4a:false};
pulse.support.activeXDetectRules = ["ShockwaveFlash.ShockwaveFlash.7", "ShockwaveFlash.ShockwaveFlash.6", "ShockwaveFlash.ShockwaveFlash"];
...
pulse.EventManager = PClass.extend({init:function(params) {
...
}, bind:function(type, callback, owner) {
  var evtName = this.checkType(type);
  var listener = {func:callback, owner:null};
  ...
  this._private.events[evtName].push(listener)
  ...
  if(this._private.events.hasOwnProperty(evtName)) {
    for(var i = this._private.events[evtName].length - 1;i >= 0;i--) {
      if(this._private.events[evtName][i].func === callback) {
  ...
  if(this.hasEvent(type)) {
    var listeners = this._private.events[type].slice();
    ...
    for(var e = 0;e < len;e++) {
      if(listeners.hasOwnProperty(e)) {
        ...
        if(listener.owner) {
          listener.func.call(listener.owner, evt)
        }else {
          listener.func(evt)
        }
      }
...
var pulse = pulse || {};
pulse.AjaxLoader = PClass.extend({init:function(method, url) {
  var self = this;
  this.request = new XMLHttpRequest;
  if("withCredentials" in this.request || typeof XDomainRequest === "undefined") {
...
pulse.Texture = pulse.Asset.extend({init:function(params) {
  ...
  this.rotation = 0;
  this.alpha = 100;
  this._private.lastSlice = {x:-1, y:-1, width:0, height:0, rotation:this.rotation, alpha:this.alpha};
  var _self = this;
...
pulse.BitmapFont = pulse.Asset.extend({init:function(params) {
  ...
  var self = this;
  var fntFile = new pulse.AjaxLoader("GET", this.filename);
  fntFile.onload(function(data, status) {
    if(status === 200) {
      var lines = data.split("\n");
      self.percentLoaded = 50;
      self.parse(lines)
    }else {
      if(status === 404) {
      }
...
pulse.Audio.Native = PClass.extend({init:function(params) {
  params = pulse.util.checkParams(params, {src:""});
  this._private = {};
  this._private.cache = {};
...
if(typeof AudioContext !== "undefined" && !pulse.Audio.ctx) {
  ...
  if(typeof webkitAudioContext !== "undefined" && !pulse.Audio.ctx) {
    pulse.Audio.ctx = new webkitAudioContext
  }
}
if(pulse.Audio.ctx) {
  window.addEventListener("touchstart", function() {
    var buffer = pulse.Audio.ctx.createBuffer(1, 1, 22050);
    var source = pulse.Audio.ctx.createBufferSource();
    source.buffer = buffer;
    source.connect(pulse.Audio.ctx.destination);
    source.noteOn(0)
  }, true)
}
;var pulse = pulse || {};
pulse.Sound = pulse.Asset.extend({init:function(params) {
  if(!params.hasOwnProperty("type")) {
    if(pulse.support.audioAPI && pulse.Sound.UseAudioAPI) {
      params.type = "api"
    }else {
      if(pulse.support.html5Audio && pulse.Sound.UseHTML5Audio) {
        params.type = "html5"
      }else {
        params.type = "flash"
      }
  ...
  if(this._private.type == "flash" && pulse.support.flash) {
    if(pulse.Sound.UseFlash) {
  ...
  this.playing = false;
  this.paused = false;
  if(this.autoLoad && (this._private.type == "api" || this._private.type == "html5" || this._private.type == "flash" && pulse.Sound.FlashReady)) {
    this.load()
  }
}, load:function() {
  var _self = this;
  hasExtension = this.filename.match(/\.\w{2,3}$/);
  switch(this._private.type) {
    case "flash":
      if(!pulse.support.flash || !pulse.Sound.UseFlash) {
    ...
    case "html5":
      ...
      this._private.audio = new pulse.Audio.Native({src:this.filename});
      this._private.audio.events.bind("error", function(evt) {
        _self.events.raiseEvent("error", evt);
        _self.percentLoaded = 100;
        _self.complete()
      });
...
pulse.Sound.UseAudioAPI = true;
pulse.Sound.UseHTML5Audio = true;
pulse.Sound.UseFlash = true;
var pulse = pulse || {};
...
pulse.Sprite = pulse.Visual.extend({init:function(params) {
  ...
  if((type === "mousedown" || type === "touchstart") && this.dragDropEnabled) {
  ...
  if((type === "mousemove" || type === "mouseup" || type === "touchstart" || type === "touchmove") && this._private.isDragging) {
...
pulse.Layer = pulse.Visual.extend({init:function(params) {
  ...
  for(var s in this.objects) {
    if(this.objects[s] instanceof pulse.Visual) {
      ...
      if(this.objects[s] && this.objects[s].updated) {
        this.updated = true
      }
    }
  }
  ...
  var sprite;
  for(var idx = this._private.orderedKeys.length - 1;idx >= 0;idx--) {
    if(evt.canceled) {
      break
    }
    sprite = this.objects[this._private.orderedKeys[idx]];
    if(!(sprite instanceof pulse.Sprite)) {
      continue
    }
    if(!sprite.visible) {
      continue
...
pulse.Scene = pulse.Node.extend({init:function(params) {
  ...
  for(var idx = this._private.orderedKeys.length - 1;idx >= 0;idx--) {
    var layer = this.layers[this._private.orderedKeys[idx]];
    if(evt.canceled) {
      break
    }
    if(pulse.events[type] === "mouse" || pulse.events[type] === "touch") {
      ...
      evt.sender = layer;
      if(layer.pointInBounds(evt.world)) {
        if((type === "mousemove" || type === "touchmove") && layer.mousein === false) {
          layer.mousein = true;
          layer.events.raiseEvent("mouseover", evt)
        }
        layer.events.raiseEvent(type, evt)
      }else {
        if(layer.mousein === true) {
          layer.mousein = false;
          layer.events.raiseEvent("mouseout", evt)
        }
      }
    }else {
      evt.sender = layer;
...
pulse.Engine = PClass.extend({init:function(params) {
  ...
  for(var s = activeScenes.length - 1;s >= 0;s--) {
