...
/** @lends pulse.Sound.prototype */
{
  ...
  init : function(params) {
    ...
    if(!params.hasOwnProperty('type')) {
      if(pulse.support.audioAPI && pulse.Sound.UseAudioAPI) {
        params.type = 'api';
      } else if(pulse.support.html5Audio && pulse.Sound.UseHTML5Audio) {
        params.type = 'html5';
      } else {
        params.type = 'flash';
      }
  ...
  load : function() {
    ...
    switch(this._private.type) {
      case 'flash':
        //If flash is not supported, complete the load
        ...
        //If flash is not ready, load when ready
        if(pulse.Sound.FlashReady === false) {
          this.autoLoad = true;
          return;
        }

        //Automatically use MP3 extension if no extension is provided
        if(!hasExtension) {
          this.filename += '.mp3';
        }
      ...
      case 'html5':
        ...
        //On any errors, propagate them and compelete loading
...
/**
 * Global property used to determine if flash sound manager is ready for use
 * initialized.
 * @type {Boolean}
 * @static
 */
...
/**
 * Global property to determine if the Audio API should be used
 * @type {Boolean}
 * @static
 */
...
/**
 * Global property to determine if HTML5 audio should be used
 * initialized.
 * @type {Boolean}
 * @static
 */
...
/**
 * Global property to determine if Flash audio should be used
 * initialized.
 * @type {Boolean}
 * @static
 */
