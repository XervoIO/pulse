...
PFPlay.Sprite = PFPlay.Visual.extend({
  init : function(params) {
    ...
    /** The radius from the center of the object to the anchor point */
    this.anchorRadius = 0;

    /** 
     * The previous and current top left position based on 
     * the position and anchor 
     */
    this.topLeft = {
      x : 0,
      y : 0
    ...
    this.update = function(elapsed) {
      ...
      if(_sheet.alpha != this.alpha) {
        _sheet.alpha = this.alpha;
      }
    }
    ...
    this.draw = function(ctx) {
      ...
      ctx.drawImage(this.getCurrentFrame(), this.topLeft.x, this.topLeft.y);
    ...
    this.getCurrentBounds = function() {
      if(PFPlay.util.getLength(_animations) > 0) {
        ...
        return {
          x : this.topLeft.x,
          y : this.topLeft.y,
          width : _sheet.width() * this.scaleX,
          height : _sheet.height() * this.scaleY
        }
      }
    };
    ...
    this.getCurrentFrame = function() {
      if(PFPlay.util.getLength(_animations) > 0) {
        ...
        return _sheet.slice(frame.x, frame.y, frame.width, frame.height);
      } else {
        return _sheet.slice();
      }
    };
    ...
    this.getPreviousBounds = function() {
      if(PFPlay.util.getLength(_animations) > 0) {
        ...
        return {
          x : _previousTopLeft.x,
          y : _previousTopLeft.y,
          width : size.x * this.scaleX,
          height : size.y * this.scaleY
        };
      } else {
