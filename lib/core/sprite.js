 ...
 * on the node
 * @config {string|PFPlay.Texture} src the texture filename to use or a 
 * PFPlay.Texture to use
...
/** @lends PFPlay.Sprite.prototype */
{
  ...
  eventsCallback : function(type, evt) {
    if(type == "mousedown" && this.dragDropEnabled) {
      ...
      PFPlay.EventManager.DraggedItems['sprite:' + this.name] = this;
    }
    ...
    if(type == "mouseup" && this._private.isDragging) {
      ...
      delete PFPlay.EventManager.DraggedItems['sprite:' + this.name];
    }
  }
});