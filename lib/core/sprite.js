...
PFPlay.Sprite = PFPlay.Visual.extend({
  init : function(params) {
    ...
    this.update = function(elapsed) {
      if(_firstUpdate) {
        _firstUpdate = false;
        this.anchorRadius = this.calculateAnchorRadius();
        return;
      }
    ...
    this.getTopLeftPosition = function() {
      ...
      var ox = cw / 2;
      var oy = ch / 2;
      var xpos = this.position.x - Math.sin(Math.PI * -this.rotation / 180) * 
                 this.anchorRadius - ox;
      var ypos = this.position.y - Math.cos(Math.PI * -this.rotation / 180) * 
