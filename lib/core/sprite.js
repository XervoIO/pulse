...
PFPlay.Sprite = function(params) 
{
  ...
  {
    name: 'sprite' + PFPlay.Sprite.spriteIdx++,
    src: ''
  });
  ...
  this.getTopLeftPosition = function()
  {
    ...
    var ox = cw / 2;
    var oy = ch / 2;
    var xpos = this.position.x - Math.sin(Math.PI * -this.rotation / 180) * this.anchorRadius - ox;
    var ypos = this.position.y - Math.cos(Math.PI * -this.rotation / 180) * this.anchorRadius - oy;
  ...
  this.eventsCallback = function(type, evt) 
  {
    ...
    if((type == "mousemove" || type == "mouseup") && _isDragging) {
      var posDiff = {x: evt.x - _dragPos.x, y: evt.y - _dragPos.y};
