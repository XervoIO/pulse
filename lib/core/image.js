...
PFPlay.Image = function(params)
{
  ...
  this.slice = function(x, y, width, height)
  {
    ...
    {
      _imgCanvas.width = width * Math.cos(this.rotation) 
        + height * Math.sin(this.rotation);
      
      _imgCanvas.width = height * Math.cos(this.rotation) 
        + width * Math.sin(this.rotation);
    }
    ...
    if(this.rotation % 360 != 0)
    {
      cxt.translate(-1 * width / 2, -1 * height / 2);
    }
