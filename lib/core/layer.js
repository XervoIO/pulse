...
PFPlay.Layer = function(name, x, y, width, height)
{
  var _name = PFPlay.util.checkValue(name, 
    'layer' + (Math.random() * 10000));
  
  this.zindex = 0;
  
  var _x = PFPlay.util.checkValue(x, 0);
  var _y = PFPlay.util.checkValue(y, 0);
  
  var _canvas = document.createElement('canvas');
  _canvas.width = PFPlay.util.checkValue(width, 0);
  _canvas.height = PFPlay.util.checkValue(height, 0);
  //_canvas.addEventListener('click', function() { alert(); }, false);
  _canvas.onclick = function() { alert(); };
  ...
  this.raiseEvent = function(type, evt)
  {
    ...
    {
      if(sprites[s].inCurrentBounds(evt.x, evt.y) && sprites[s].hasEvent(type))
      {
        sprites[s].raiseEvent(type, evt);
      }
    }
  };
};

PFPlay.Layer.prototype.eventClick = function(evt)
{
