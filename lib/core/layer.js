...
PFPlay.Layer = Node.extend({
   init: function(params){
     ...
     this.update = function(elapsed)
     {
       ...
       if(reorder)
         _orderedKeys = PFPlay.util.getOrderedKeys(_objects);
     ...
     {    
       ...
       if(isDirty) {
         ...
         for(var o = 0; o < _orderedKeys.length; o++) {
           
           var obj = _objects[_orderedKeys[o]];
           
           if(obj instanceof PFPlay.Sprite)
           {
             if(obj.visible && obj.loaded()) {
               obj.draw(_ctx);
             }
           }
         }
     ...
     this.events = new PFPlay.EventManager({masterCallback:
       function(type, evt) {
         ...
         for(var s in sprites)
         {
           ...
           {
             var sBounds = sprite.getCurrentBounds();
             
             evt.position.x = evt.world.x - sBounds.x;
             evt.position.y = evt.world.y - sBounds.y;
             
             if(sprite.handleAllEvents || sprite.inCurrentBounds(evt.world.x, evt.world.y))
             {
             ...
             if(sprite.dropAcceptEnabled) 
             {
               for(var id in PFPlay.EventManager.DraggedItems) 
               {
                 ...
                 if(sprite.inCurrentBounds(evt.world.x, evt.world.y))
                 {
