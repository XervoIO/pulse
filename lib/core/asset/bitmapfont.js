 ...
 */
PFPlay.BitmapChar = PClass.extend(
/** @lends PFPlay.BitmapChar.prototype */
{
  /** @constructs */
  init : function() {
    /**
 ...
 * @copyright 2011 Paranoid Ferret Productions
 */
PFPlay.BitmapFont = PFPlay.Asset.extend(
/** @lends PFPlay.BitmapFont.prototype */
{
  /** @constructs */
  init : function(params) {
     ...
     * and value is PFPlay.BitmapChar.
     * @type {array}
     */
    this.characters = {};

    // Parse the directory from the font file
    var pathParts = this.filename.split('/');
    pathParts.pop();
  ...
  parse : function(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if(line.indexOf("common") == 0) {
        ...
        var character = new PFPlay.BitmapChar();
        var keyvals = line.split(" ");
        for (var j = 0; j < keyvals.length; j++) {
          var keyval = keyvals[j].split("=");
          if(keyval.length > 1) {
